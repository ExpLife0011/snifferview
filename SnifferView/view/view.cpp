...
VOID OnAnalysisDumpFile(const char *file)
{
	...
	if(DumpPacketsFromFile(file, g_filter_packets))
	{
		vv.format("SnifferView - 数据分析\\\\%hs", file);
		SetWindowTextA(g_main_view, vv.c_str());
		g_filter_count = g_filter_packets.size();
		RecheckShowPacket();
...
static VOID WINAPI ShowPacketData(int itm)
{
	...
	//LOCK_SHOW;
	LOCK_FILTER;
	if (itm < (int)g_show_packets.size())
	{
		int hight_begin = 0;
		int hight_end = 0;
		int mark = 0;
		SetData((const BYTE *)g_show_packets[itm]->m_packet.c_str(), g_show_packets[itm]->m_packet.size());
		if (g_show_packets[itm]->m_tls_type == em_tls_tcp)
		{
			//mark = sizeof(TCPHeader);
			TCPHeader *tcp = (TCPHeader *)(g_show_packets[itm]->m_packet.c_str() + sizeof(IPHeader));
			mark = tcp->get_tcp_header_length();
		}
		else if (g_show_packets[itm]->m_tls_type == em_tls_udp)
		{
			mark = sizeof(UDPHeader);
		}
		else if (g_show_packets[itm]->m_tls_type == em_tls_icmp)
		{
		...
		else if (g_hex_hight == em_user_data)
		{
			...
			hight_end = g_show_packets[itm]->m_packet.size() - 1;
		}
...
VOID WINAPI OnExportFile()
{
    ...
    do 
    {
        if (ShowFileSaveDialog(g_main_view, name.c_str(), g_def_dir.c_str(), file))
        {
            ...
            LOCK_FILTER;
            BOOL ret = DumpPacketsToFile(file.c_str(), g_filter_packets);
            UNLOCK_FILTER;
            file.repsub("/", "\\");
            mstring dir = file;
            int m = dir.rfind('\\');
...
VOID WINAPI OnGetListCtrlDisplsy(IN OUT NMLVDISPINFO* plvdi)
{
	...
	//LOCK_SHOW;
	LOCK_FILTER;
	if ((int)g_show_packets.size() <= itm)
	{
		//UNLOCK_SHOW;
		UNLOCK_FILTER;
		return;
	}
	tmp = g_show_packets[itm];
	...
	case CDDS_ITEMPREPAINT:
		{
			...
			LOCK_FILTER;
			col = g_show_packets[itm]->m_colour;
			UNLOCK_FILTER;
...
VOID OnSelectChange(WPARAM wp, LPARAM lp)
{
    ...
    {
        if (s_last_select < 0 || s_last_select >= (int)g_show_packets.size())
        {
            goto leave;
        }

        if (g_show_packets[s_last_select]->m_packet.size() <= (size_t)end)
        {
            goto leave;
        }
        ...
        {
            d <<= 8;
            d |= (BYTE)g_show_packets[s_last_select]->m_packet.c_str()[itm];
        }
...
DWORD CALLBACK ViewProc(HWND hdlg, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case  MSG_UPDATE_DATA:
		{
			LOCK_FILTER;
			SendMessageA(s_list, LVM_SETITEMCOUNT, g_show_packets.size(), LVSICF_NOSCROLL | LVSICF_NOINVALIDATEALL);
			UNLOCK_FILTER;
		}
		break;
...
VOID WINAPI UpdatePacketsCount()
{
	if (IsWindow(s_status))
	{
		...
		if(g_work_state == em_sniffer)
		{
			v.format("封包总数：%d  符合过滤：%d  符合显示：%d", g_packet_count, g_filter_count, g_show_count);
		}
		else if (g_work_state == em_analysis)
		{
            v.format("封包总数：%d  符合显示：%d", g_filter_count, g_show_count);
		}
