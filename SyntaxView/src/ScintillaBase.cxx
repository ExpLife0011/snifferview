...
class LexState : public LexInterface {
	...
	void SetLexerModule(LexerModule *lex);
	PropSetSimple props;
...
public:
    ...
    void SetLexerLabel(const std::string &label);
};
...
void LexState::SetLexerModule(LexerModule *lex) {
	if (lex != lexCurrent) {
		if (instance) {
			instance->Release();
			instance = 0;
...
void LexState::SetLexer(uptr_t wParam) {
	...
	if (lexLanguage == SCLEX_CONTAINER) {
        ...
        LexerModule *lex = Catalogue::Find(lexLanguage);
		if (!lex)
...
void LexState::SetLexerLanguage(const char *languageName) {
	LexerModule *lex = Catalogue::Find(languageName);
	if (!lex)
		lex = Catalogue::Find(SCLEX_NULL);
	if (lex)
